...
var skip = 0;
var frameskip = 4; // this will make the game run at only ~15-20 FPS but values <4 will drop even more frames
...
function reportOnGamepad() {
	...
	for(var i=0;i<gp.buttons.length;i++) {			
		if(gp.buttons[i].pressed){ //req.send("key=" +i + " pressed");			
			switch (i){
				case 14:
					if (!left_pressed){
					sendKey("left_down"); //req.send("key=" +"left");
					left_pressed = true;};
					break;
				case 12:
					if (!up_pressed){
